    ...
    for pair in root.into_inner() {
        match pair.as_rule() {
            Rule::definition => {
                ...
                match inner.as_rule() {
                    Rule::value_definition => definitions.push(Definition::Value(parse_value_def(inner)?)),
                    Rule::period_definition => definitions.push(Definition::Period(parse_period_def(inner)?)),
                    Rule::plan_definition => definitions.push(Definition::Plan(parse_plan_def(inner)?)),
                    Rule::drug_definition => definitions.push(Definition::Drug(parse_drug_def(inner)?)),
                    Rule::addressee_definition => definitions.push(Definition::Addressee(parse_addressee_def(inner)?)),
                    Rule::context_definition => definitions.push(Definition::Context(parse_context_def(inner)?)),
                    _ => (),
                }
            }
            Rule::EOI => (),
...
fn parse_value_def(pair: pest::iterators::Pair<Rule>) -> Result<ValueDef, ParseError> {
    ...
    Ok(ValueDef { name, value_type, properties })
}
...
fn parse_period_def(pair: pest::iterators::Pair<Rule>) -> Result<PeriodDef, ParseError> {
    ...
    Ok(PeriodDef { name, timeframes: vec![] })
}
...
fn parse_plan_block(pair: pest::iterators::Pair<Rule>) -> Result<PlanBlock, ParseError> {
    ...
    match inner.as_rule() {
        ...
        Rule::event_block => {
             ...
             let stmts = parse_block(block_body)?;
             Ok(PlanBlock::Event(EventBlock { name, trigger, statements: stmts }))
        }
        Rule::trigger_block => {
             ...
             let stmts = parse_block(block_body)?;
             Ok(PlanBlock::Trigger(TriggerBlock { trigger, statements: stmts }))
        }
        ...
        match p.as_rule() {
            Rule::ingredients_block => {
                ...
                for i_pair in p.into_inner() {
                     ...
                     let i_amt = i_inner.next().unwrap().as_str().trim().parse::<f64>().unwrap_or(0.0);
                     let i_unit = parse_unit(i_inner.next().unwrap())?;
                     ingredients.push(Ingredient { name: i_name, amount: i_amt, unit: i_unit });
                }
            ...
            Rule::dosage_block => {
                 ...
                 for r_pair in p.into_inner() {
                      ...
                      let rule_type = r_inner.next().unwrap().as_str(); // "maximum single dose" etc
                      // Wait, pest returns string literal? rule_type is atomic part of rule? 
                      // dosage_rule = { ("max..." | "max...") ~ "=" ~ expression }
                      // inner contains string literal for type and expression.
                      
                      let expr = parse_expression(r_inner.next().unwrap())?;
                      if rule_type.starts_with("maximum single") {
                          rules.push(DosageRule::MaxSingle(expr));
                      } else if rule_type.starts_with("maximum daily") {
            ...
            Rule::admin_block => {
                 ...
                 for admin_pair in p.into_inner() {
                      let mut a_inner = admin_pair.into_inner();
                      // admin_rule alts: form or schedule
                      let first = a_inner.next().unwrap();
                      if first.as_str().starts_with("form of") {
                           let ident = parse_identifier_str(a_inner.next().unwrap());
                           rules.push(AdminRule::Form(ident));
                      } else {
                           // identifier ~ quantity ~ "after" ~ identifier
                           // first is identifier (drug name1)
                           let drug1 = parse_identifier_str(first);
                           let qty_expr = parse_expression(a_inner.next().unwrap())?; // Quantity expression
                           let drug2 = parse_identifier_str(a_inner.next().unwrap());
            ...
            Rule::interaction_block => {
                 ...
                 for i_pair in p.into_inner() {
                      ...
                      rules.push(InteractionRule { drug: drug_name, block });
                 }
...
fn parse_addressee_def(pair: pest::iterators::Pair<Rule>) -> Result<AddresseeDef, ParseError> {
        ...
        match p.as_rule() {
            Rule::contact_info_prop => {
                 ...
                 for c in p.into_inner() {
                      ...
                      if type_str == "email" { details.push(ContactDetail::Email(val)); }
                      else if type_str == "phone" { details.push(ContactDetail::Phone(val)); }
                      else { details.push(ContactDetail::HippocratesId(val)); }
                 }
    ...
    Ok(AddresseeDef { name, is_group, properties })
}
    ...
    for p in pair.into_inner() {
        if p.as_rule() == Rule::context_item {
             ...
             if s.starts_with("timeframe") {
                  ...
                  for child in i_inner {
                       if child.as_rule() == Rule::identifier || child.as_rule() == Rule::string_literal {
...
fn parse_quantity_pair(pair: pest::iterators::Pair<Rule>) -> Result<(f64, Unit), ParseError> {
    ...
    let val = number_pair.as_str().parse::<f64>().map_err(|e| ParseError::PestError(pest::error::Error::new_from_span(pest::error::ErrorVariant::CustomError { message: e.to_string() }, number_pair.as_span())))?;
...
fn parse_assessment_cases(pairs: pest::iterators::Pairs<Rule>) -> Result<Vec<AssessmentCase>, ParseError> {
...
fn parse_valid_values_block(pair: pest::iterators::Pair<Rule>) -> Result<Vec<RangeSelector>, ParseError> {
...
fn parse_assessment_case(pair: pest::iterators::Pair<Rule>) -> Result<Vec<AssessmentCase>, ParseError> {
    let mut inner = pair.into_inner();
    ...
    for sel in selector_list.into_inner() {
         match sel.as_rule() {
             ...
             _ => return Err(ParseError::UnknownRule(format!("Unexpected selector rule: {:?}", sel.as_rule()))),
         }
    }
    ...
    for cond in selectors {
         cases.push(AssessmentCase { condition: cond, block: block.clone() });
    }
...
fn parse_range_selector(pair: pest::iterators::Pair<Rule>) -> Result<RangeSelector, ParseError> {
    ...
    if s.starts_with(">") {
         return Ok(RangeSelector::GreaterThan(parse_expression(first)?));
    }
...
fn parse_show_message(pairs: pest::iterators::Pairs<Rule>) -> Result<Action, ParseError> {
    ...
    Ok(Action::ShowMessage(message_parts, if statements.is_empty() { None } else { Some(statements) }))
}
...
fn parse_ask_question(mut pairs: pest::iterators::Pairs<Rule>) -> Result<Action, ParseError> {
    ...
    let opt_stmts = if statements.is_empty() { None } else { Some(statements) };
    ...
    match pair.as_rule() {
        ...
        Rule::statistical_func => {
               ...
               if s.starts_with("count of") {
                   ...
                   Ok(Expression::Statistical(StatisticalFunc::CountOf(ident, filter)))
                    ...
                    Ok(Expression::Statistical(StatisticalFunc::AverageOf(ident, Box::new(q_expr))))
               } else {
