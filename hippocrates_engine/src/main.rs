...
fn main() {
    ...
    match parser::parse_plan(&content) {
        Ok(plan) => {
            ...
            engine.env.set_output_handler(std::sync::Arc::new(|msg: String| {
                println!("{}", msg);
            }));

            if let Some(plan_name) = plan.definitions.iter().find_map(|d| {
                match d {
                ...
                engine.set_value("empty bottles", hippocrates_engine::domain::RuntimeValue::Number(0.0));
